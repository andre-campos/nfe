...
class WSLoteEnvio {
	...
	NFLoteEnvioRetorno enviaLoteAssinado(final String loteAssinadoXml,
			final NFModelo modelo) throws Exception {
	...
	NFLoteEnvioRetornoDados enviaLote(final NFLoteEnvio lote) throws Exception {
		...
		for (final NFNota nota : lote.getNotas()) {
			...
			nota.getInfo()
					.getIdentificacao()
					.setCodigoRandomico(
							StringUtils.defaultIfBlank(nota.getInfo()
									.getIdentificacao().getCodigoRandomico(),
									geraChave.geraCodigoRandomico()));
			nota.getInfo().getIdentificacao()
					.setDigitoVerificador(geraChave.getDV());
			nota.getInfo().setIdentificador(geraChave.getChaveAcesso());
		}

		// assina o lote
		final String documentoAssinado = new AssinaturaDigital(this.config)
				.assinarDocumento(lote.toString());
		final NFLoteEnvio loteAssinado = new NotaParser()
				.loteParaObjeto(documentoAssinado);

		// verifica se nao tem NFCe junto com NFe no lote e gera qrcode (apos
		// assinar mesmo, eh assim)
		...
		for (final NFNota nota : loteAssinado.getNotas()) {
			...
			case NFE:
				...
				break;
			case NFCE:
				final NFGeraQRCode geraQRCode = new NFGeraQRCode(nota,
						this.config);
			...
			default:
				throw new IllegalArgumentException(String.format(
						"Modelo de nota desconhecida: %s", nota.getInfo()
								.getIdentificacao().getModelo()));
			}
		}
		...
		// comunica o lote
		final NFLoteEnvioRetorno loteEnvioRetorno = this.comunicaLote(
				loteAssinado.toString(), modelo);
		return new NFLoteEnvioRetornoDados(loteEnvioRetorno, loteAssinado);
	}

	private NFLoteEnvioRetorno comunicaLote(final String loteAssinadoXml,
			final NFModelo modelo) throws Exception {
		...
		final NFAutorizador31 autorizador = NFAutorizador31.valueOfTipoEmissao(
				this.config.getTipoEmissao(), this.config.getCUF());

		final String endpoint = NFModelo.NFE.equals(modelo) ? autorizador
				.getNfeAutorizacao(this.config.getAmbiente()) : autorizador
				.getNfceAutorizacao(this.config.getAmbiente());
		if (endpoint == null) {
			throw new IllegalArgumentException(
					"Nao foi possivel encontrar URL para Autorizacao "
							+ modelo.name() + ", autorizador "
							+ autorizador.name());
		}

		final NfeAutorizacaoLoteResult autorizacaoLoteResult = new NfeAutorizacaoStub(
				endpoint).nfeAutorizacaoLote(dados, cabecalhoSOAP);
		try {
			final NFLoteEnvioRetorno loteEnvioRetorno = new NFPersister().read(
					NFLoteEnvioRetorno.class, autorizacaoLoteResult
							.getExtraElement().toString());
			WSLoteEnvio.LOGGER.info(loteEnvioRetorno.toString());
			return loteEnvioRetorno;
		} catch (Exception e) {
			WSLoteEnvio.LOGGER.error(autorizacaoLoteResult.getExtraElement()
					.toString());
			throw new UnexpectedException(autorizacaoLoteResult
					.getExtraElement().toString(), e);
		}
	}
	...
	private OMElement nfeToOMElement(final String documento)
			throws XMLStreamException {
		final OMElement ome = AXIOMUtil.stringToOM(documento);
		final Iterator<?> children = ome
				.getChildrenWithLocalName(WSLoteEnvio.NFE_ELEMENTO);
		while (children.hasNext()) {
			final OMElement omElement = (OMElement) children.next();
			if ((omElement != null)
					&& (WSLoteEnvio.NFE_ELEMENTO.equals(omElement
							.getLocalName()))) {
